...
public class GetMedications : ICarterModule
{
    ...
    {
        app.MapGet("/api/v1/medications", async ([AsParameters] PaginationRequest paginationRequest, ISender sender) =>
        {
            var result = await sender.Send(new GetMedicationsQuery(paginationRequest));
